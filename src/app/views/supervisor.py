...
def show():
    ...
    try:
        ...
        ax.set_title("Red nodes indicate bottleneck predictions for the next 5 units (~15-30m)", fontsize=12, color="gray")
        st.pyplot(fig)
        
        st.markdown("---")
        st.subheader("Station Telemetry")
        st.dataframe(curr_state[['Station_ID', 'Coverage', 'Inferred_Time', 'Predicted_Time', 'Risk_Score']].reset_index(drop=True), width='stretch')
